...
import { 
  X, Star, ShieldCheck, Heart, ShoppingBag, Share2, BookOpen, Clock, 
  HelpCircle, Sparkles, Plus, AlertCircle, Download, Check, RefreshCcw 
} from "lucide-react";
...
interface ProductDetailModalProps {
  product: Product;
  onClose: () => void;
  onAddToCart: (product: Product, quantity: number, subscriptionType: "once" | "monthly" | "bi-monthly") => void;
...
export default function ProductDetailModal({
  ...
  const bundleTotalPrice = Math.round((product.price + bundleProduct.price) * 0.9); // 10% discount

  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-stone-900/60 backdrop-blur-md flex items-center justify-center p-4">
      <div className="relative w-full max-w-5xl rounded-3xl bg-white shadow-2xl overflow-hidden border border-amber-100 max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between border-b border-stone-100 bg-stone-50 px-6 py-4">
          ...
          <button 
            onClick={onClose}
            className="rounded-full bg-stone-200/80 p-2 text-stone-500 hover:bg-stone-300 hover:text-stone-800 transition-colors cursor-pointer"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
              ...
              <div className="flex items-center gap-4">
                <div className="flex h-11 items-center rounded-xl border border-stone-300 bg-white">
                  <button
                    ...
                    className="px-3.5 py-2 text-stone-600 hover:text-stone-900 font-bold"
                  >
                    -
                  </button>
                ...
                <button
                  onClick={() => onAddToCart(product, quantity, subscriptionType)}
                  className="flex-1 flex h-11 items-center justify-center gap-2 rounded-xl bg-emerald-800 text-white font-bold hover:bg-emerald-950 transition-colors cursor-pointer shadow-md"
                >
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 justify-between">
              ...
              <div className="flex items-center gap-4">
                ...
                {bundleAdded ? (
                  ...
                  <button
                    onClick={() => {
                      onAddToCart(product, 1, "once");
                      onAddToCart(bundleProduct, 1, "once");
                      setBundleAdded(true);
                    }}
